...
int gr_light_cmd(lua_State* L)
{
  ...
  int lightType = luaL_checknumber(L, 4);
  Light l;
...
int gr_render_cmd(lua_State* L)
{
  ...
  double focal = luaL_checknumber(L, 11);
  int samp = luaL_checknumber(L, 12);

  luaL_checktype(L, 13, LUA_TTABLE);

  int texture_count = int(lua_rawlen(L, 13));

  std::vector<std::string> textureLib;
  for (int i = 1; i <= texture_count; i++) {
    lua_rawgeti(L, 13, i);
    textureLib.push_back(luaL_checkstring(L, -1));
  }

	Image im( width, height);
	A5_Render(root->node, im, eye, view, up, fov, focal, samp, ambient, lights, textureLib);
    im.savePng( filename );
...
int gr_material_cmd(lua_State* L)
{
  ...
  int texture = luaL_checknumber(L, 6);
  double bump = luaL_checknumber(L, 7);
